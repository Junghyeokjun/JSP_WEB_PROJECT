  ...
  <div class="d-flex flex-column justify-content-between" style="height: 100vh;">
    ...
    <div class="flex-grow-1 d-flex flex-column" style="background-color: #cdcdcd;" id="main">
      ...
      <!-- table end -->
        ...
        <h1 class="my-5">수강정보조회</h1>
        <table class="table-bordered  w-75 mx-auto ">
          <thead >
            <tr class="fs-4">
              <th class="fw-bold py-1" scope="col">수강월</th>
              <th class="fw-bold" scope="col">회원번호</th>
              <th class="fw-bold" scope="col">회원명</th>
              <th class="fw-bold" scope="col">강의명</th>
              <th class="fw-bold" scope="col">강의장소</th>
              <th class="fw-bold" scope="col">수강료</th>
              <th class="fw-bold" scope="col">등급</th>
            </tr>
          </thead>
          <tbody>
            <c:forEach var="sineUp" items="${sineUpList}">
            	<tr class="fs-4">
	            	<td class="py-1">${sineUp.registMonth}</td>
	            	<td>${sineUp.cNo}</td>
	            	<td>${sineUp.cName}</td>
	            	<td>${sineUp.className}</td>
	            	<td>${sineUp.classArea}</td>
	            	<td>\<span class="price">${sineUp.tuition}<span></td>
	            	<td>${sineUp.grade}</td>
            	</tr>
            </c:forEach>
          </tbody>
